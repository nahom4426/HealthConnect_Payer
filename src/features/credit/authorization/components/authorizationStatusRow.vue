...
import { changeInsuredStatus, getPayerbyPayerUuid } from "../api/authorizationApi";
...
function handleEdit(row) {
  if (row.insuredUuid) {
    openModal('EditInsured', { 
      ...
      insured: row,
      onUpdated: updated => authorization.update(updated.insuredUuid, updated)
    });
...
<template>
  ...
  >  
    ...
    <!-- <td class="p-4 font-medium text-gray-500">{{ rowData.index }}</td>   -->
 <td class="p-4" @click.stop>
      <input
        type="checkbox"
        :checked="isSelected"
        @change="onSelect(rowData.claimUuid)"
