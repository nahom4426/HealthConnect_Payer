...
import InsuredForm from "../form/authorizationForm.vue";
import Button from "@/components/Button.vue";
import { useRouter } from 'vue-router';
import { closeModal } from "@customizer/modal-x";
import { toasted } from "@/utils/utils";
import { ref } from "vue";
import InsuredPersonFormDataProvider from "../form/authorizationFormDataProvider.vue";
...
async function handleSubmit(formValues: any) {
  try {
    ...
    if (result.success) {
      ...
      const newInsured = {
        ...result.data,
        photoUrl: result.data.photoUrl || (result.data.photoPath ? 
          `${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/insured/photo/${result.data.photoPath}` : 
          null)
      };
      
      // Add the new insured to the store
      authorization.add(newInsured);
