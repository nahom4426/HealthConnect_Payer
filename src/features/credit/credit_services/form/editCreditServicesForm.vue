  ...
  try {
    ...
    employees.value = employeesData.flatMap((emp) => {
      const employeeRecord = {
        ...
        fullName: `${emp.firstName || ""} ${emp.fatherName || ""} ${
          emp.grandFatherName || ""
        }`.trim(),
        phone: emp.phone,
        email: emp.email || "N/A",
      ...
      const dependantRecords =
        emp.dependants?.map((dep) => ({
          insuredUuid: dep.dependantUuid,
          fullName: `${dep.dependantFirstName || ""} ${
            dep.dependantFatherName || ""
          } ${dep.dependantGrandFatherName || ""}`.trim(),
          phone: emp.phone,
          email: emp.email || "N/A",
...
function handleAddItem(item) {
  (activeTab.value === "services" ? addedServices : addedDrugs).value.push(
    item
  );
}
function handleRemoveItem(index) {
  (activeTab.value === "services" ? addedServices : addedDrugs).value.splice(
    index,
    1
  );
}
function handleUpdateQuantity({ index, value }) {
  (activeTab.value === "services" ? addedServices : addedDrugs).value[
    index
  ].quantity = value;
}
function handleUpdateDiagnosis({
  index,
...
function validateForm() {
  ...
  const hasItems =
    addedServices.value.length > 0 || addedDrugs.value.length > 0;
  if (!hasItems) {
    error.value = "Please add at least one service or drug";
    return false;
  }

  const invalidDrug = addedDrugs.value.some(
    (item) => !item.route || !item.frequency || !item.dose || !item.duration
  );
  if (invalidDrug) {
...
async function handleSubmit() {
  ...
  try {
    ...
    const payload = {
      ...
      dependantUuid: selectedEmployee.value.isDependant
        ? selectedEmployee.value.dependantUuid
        : null,
      ...
      medicationItems: [
        ...addedServices.value.map((item) => ({
          ...
          secondaryDiagnosis:
            item.secondaryDiagnosis || secondaryDiagnosis.value,
        })),
    ...
    const response = await updateDispensingRecord(
      props.dispensingUuid,
      payload
    );
    if (response?.success) {
      ...
      throw new Error(
        response?.message ||
          response?.error ||
          "Update failed without error message"
      );
    }
  } catch (err) {
    const errorMessage =
      err.response?.data?.message || err.message || "Failed to update claim";
      ...
      if (claimData.value.items) {
        for (const item of claimData.value.items) {
          const baseItem = {
            ...
            primaryDiagnosis:
              item.primaryDiagnosis || claimData.value.primaryDiagnosis || "",
            secondaryDiagnosis:
              item.secondaryDiagnosis ||
              claimData.value.secondaryDiagnosis ||
              "",
          };
    ...
    <template v-else>
      ...
      <div v-if="currentStep === 'selectEmployee'" class="py-3 space-y-6">
            ...
            <div class="border rounded-lg">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  >
                    <tr
                      v-if="!employee.isDependant"
                      :class="{
                        'bg-[#DFF1F1]':
                          selectedEmployee &&
                          selectedEmployee.insuredUuid === employee.insuredUuid,
                        'border-b-2 border-blue-200':
                          employee.dependants && employee.dependants.length > 0,
                      }"
                    >
                      <td
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-bold">
                        <span
                          :class="
                            employee.eligible
                              ? 'bg-[#DFF1F1] text-[#02676B] p-1'
                              : 'text-[#DB2E48] bg-[#DB2E481A] p-1'
                          "
                        >
                        ...
                        <button
                          ...
                          :class="{
                            'text-white bg-[#02676B] px-4 py-2 hover:bg-teal-900':
                              !(
                                selectedEmployee &&
                                selectedEmployee.insuredUuid ===
                                  employee.insuredUuid
                              ) && employee.eligible,
                      ...
                      <tr
                        ...
                        :class="{
                          'bg-blue-100':
                            selectedEmployee &&
                            selectedEmployee.insuredUuid ===
                              dependant.dependantUuid,
                        }"
                      >
                        <td
                          class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 pl-10"
                        >
                        ...
                        <td
                          class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 pl-2"
                        >
                          {{
                          ...
                          <span class="text-xs text-blue-600 ml-1"
                            >(Dependant - {{ dependant.relationship }})</span
                          >
                        </td>
                        <td
                          class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
        ...
        <div
          ...
          class="pt-4 px-6 border-t border-[#DFDEF2] flex justify-end space-x-4"
        >
          <Button
            type="button"
      ...
      <div v-else-if="employeeDetails" class="py-3 space-y-6">
        ...
        <selectServices
          ...
          :search-query="
            activeTab === 'services' ? searchServiceQuery : searchDrugQuery
          "
          :pending="fetchPending"
          :available-items="
            activeTab === 'services' ? availableServices : availableDrugs
          "
          ...
          @update:search-query="
            activeTab === 'services'
              ? (searchServiceQuery = $event)
              : (searchDrugQuery = $event)
          "
