  ...
  try {
    ...
    try {
      if (formValues.medicationItems) {
        ...
        result = await updateCreditService(
          dispensingUuid.value,
          servicePayload
        );
      ...
      if (result.success) {
        ...
        updatedData = {
          ...commonPayload,
          invoiceNumber:
            result.data?.invoiceNumber || claimData.value.invoiceNumber,
          ...
          totalAmount: formValues.medicationItems
            ...
            : formValues.drugItems.reduce(
                (sum: number, item: any) =>
                  sum + (item.price || 0) * (Number(item.quantity) || 1),
                0
              ),
        ...
        if (
          ...
          typeof props.data.onUpdated === "function"
        ) {
  ...
  <ModalParent>
    ...
    >
      <div class="bg-white rounded-lg">
        ...
        <div
          v-if="loading"
          class="p-4 mb-4 text-sm text-blue-700 bg-blue-100 rounded-lg flex items-center gap-2"
        >
