    ...
    if (formValues.medicationItems) {
      ...
      const drugPayload = {
        ...
        drugItems: formValues.drugItems.map((item: any) => ({
          ...
          totalPrice:
            item.totalPrice || (item.price || 0) * (Number(item.quantity) || 1),
      ...
      throw new Error(
        "Invalid items format - must have either medicationItems or drugItems"
      );
    }
    ...
    if (result.success) {
      ...
      const storeData = {
        ...
        totalAmount: formValues.medicationItems
          ? formValues.medicationItems.reduce(
              (sum: number, item: any) =>
                sum +
