...
function formatName(person: any) {
  return `${person.insuredTitle || ""} ${person.firstName || ""} ${
    person.fatherName || ""
  } ${person.grandFatherName || ""}`.trim();
}
...
async function saveChanges() {
  try {
    loading.value = true;
    const response = await updateInsuredPerson(
      editedPerson.value.insuredPersonUuid,
      editedPerson.value
    );
...
<template>
  <div
    v-if="loading"
    class="text-center py-12 text-lg font-semibold text-gray-600 animate-pulse"
  >
    Loading...
  </div>
  ...
  <div
    v-else-if="error"
    class="text-center py-8 text-red-600 text-lg font-semibold"
  >
    {{ error }}
  </div>
  ...
  >
    <div
      class="px-6 py-5 bg-blue-50 border-b border-gray-200 flex justify-between items-center"
    >
    ...
    <div class="p-6 space-y-10">
        ...
        <div>
          ...
          <div class="space-y-4">
            <template v-if="isEditing">
              ...
              <Select
                v-model="editedPerson.insuredTitle"
              ...
              />
              <Input
                v-model="editedPerson.fatherName"
                label="Father's Name"
                name="fatherName"
                :attributes="{
              ...
              />
              <Input
                v-model="editedPerson.grandFatherName"
              ...
              />
              <Input
                v-model="editedPerson.birthDate"
                type="date"
                label="Birth Date"
                name="birthDate"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          ...
          <div class="space-y-2">
            <template v-if="isEditing">
              <Input
                v-model="editedPerson.insuranceId"
                label="Insurance ID"
                name="insuranceId"
                :attributes="{
            ...
            <template v-else>
              <p class="text-sm text-gray-700">
                🆔 <span class="font-medium">Insurance ID:</span>
                {{ person.insuranceId }}
              </p>
              <p class="text-sm text-gray-700">
                🔑 <span class="font-medium">Insured UUID:</span>
                {{ person.insuredPersonUuid }}
              </p>
            </template>
          </div>
        </div>
          ...
          <div>
            ...
            <template v-else>
              <span
                ...
                :class="getStatusStyle(person.status)"
              >
                {{ person.status }}
              </span>
            </template>
      ...
      <div>
        ...
        <div
          v-if="editedPerson.dependants && editedPerson.dependants.length > 0"
        ...
        >
          ...
          >
            <template v-if="isEditing">
              ...
              <Input
                v-model="dependant.dependantFirstName"
                label="First Name"
                :name="`dependant-${index}-firstName`"
                :attributes="{ class: 'w-full input-compact' }"
              />
              <Input
                v-model="dependant.dependantFatherName"
                label="Father's Name"
                :name="`dependant-${index}-fatherName`"
                :attributes="{ class: 'w-full input-compact' }"
              />
              <Input
                v-model="dependant.dependantGrandFatherName"
                label="Grandfather's Name"
                :name="`dependant-${index}-grandFatherName`"
                :attributes="{ class: 'w-full input-compact' }"
              />
              <Input
                v-model="dependant.relationship"
                label="Relationship"
                :name="`dependant-${index}-relationship`"
                :attributes="{ class: 'w-full input-compact' }"
              />
              <Select
                v-model="dependant.dependantGender"
                :options="['Male', 'Female']"
                label="Gender"
                :name="`dependant-${index}-gender`"
                :attributes="{ class: 'w-full input-compact' }"
              />
              <Input
                v-model="dependant.dependantBirthDate"
                type="date"
                label="Birth Date"
                :name="`dependant-${index}-birthDate`"
                :attributes="{ class: 'w-full input-compact' }"
              />
              <Select
                v-model="dependant.dependantStatus"
                :options="['ACTIVE', 'INACTIVE', 'SUSPENDED', 'PENDING']"
                label="Status"
            ...
            <template v-else>
              <h4 class="font-semibold text-gray-900">
                {{ dependant.dependantFirstName }}
                {{ dependant.dependantFatherName }}
                {{ dependant.dependantGrandFatherName }}
              </h4>
              <p class="text-sm text-gray-700">
              ...
              <p class="text-sm text-gray-700">
                Birth Date: {{ formatDate(dependant.dependantBirthDate) }}
              </p>
              <p class="text-sm text-gray-700">
