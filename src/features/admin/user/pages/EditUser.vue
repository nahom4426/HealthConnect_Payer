...
import { allRequest, toasted } from '@/utils/utils.ts';
    ...
    (response) => {
        ...
        if (response && response.data) {
            ...
            if (response.data.roles && Array.isArray(response.data.roles)) {
                ...
                rolesData.value = response.data.roles.map(role => {
...
<template>
    ...
    <NewFormParent size="xl" title="Update Users">
        ...
        <div v-else-if="!userData || rolesData.length === 0" class="p-6 text-center text-red-500">
            Failed to load user data. Please try again.
            <pre v-if="process.env.NODE_ENV === 'development'" class="mt-4 text-xs text-left bg-gray-100 p-2 rounded">
        ...
        />
        <template #bottom>
            <div class="p-2" v-if="!userReq.pending.value && userData && rolesData.length > 0">
                <Button 
                    type="primary" 
