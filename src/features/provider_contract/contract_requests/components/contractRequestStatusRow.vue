...
const props = defineProps({
  ...
  cells: {
    // Add this to handle the cells prop
    type: Array,
    default: () => [],
  },
  onView: {
...
async function handleActivateWithClose(contract) {
  closeAllDropdowns();
  try {
    const response = await changePayerContractStatus(
      contract.contractHeaderUuid,
      "ACTIVE"
    );
    if (response.success) {
      addToast({
      ...
      payerContractsStore.update(contract.contractHeaderUuid, {
        status: "ACTIVE",
      });
    } else {
    ...
    addToast({
      ...
      message:
        error.message || "An error occurred while activating the contract",
    });
  }
}
...
async function handleDeactivateWithClose(contract) {
  ...
  try {
    ...
    addToast({
      ...
      message:
        error.message || "An error occurred while deactivating the contract",
    });
  }
}
</script>
  ...
  <template v-for="(row, idx) in rowData" :key="idx">
    ...
    >
      ...
      <td class="p-3 py-4" v-for="key in rowKeys" :key="key">
        ...
        <div
          v-else-if="key === 'contractName'"
          class="text-gray-700 flex items-center gap-2.5"
        >
          {{ row.contractName }}
        </div>
      ...
      >
        <div class="dropdown-container relative w-full">
          <button
            @click.stop="
              toggleDropdown($event, row.contractHeaderUuid || row.id)
            "
