...
function applyPercentageDiscount() {
  ...
  selectedItems.value.forEach((item) => {
    item.userPrice = parseFloat(
      (item.providerPrice * discountFactor).toFixed(2)
    );
  });

  if (activeTab.value === "services") {
    services.value = services.value.map((service) => {
      const selectedItem = selectedItems.value.find(
        (si) => si.id === service.id
      );
      if (selectedItem) {
      ...
      <div class="grid md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 bg-[#F6F7FA] rounded">
          <div class="flex items-center p-4 justify-between">
            <h3 class="text-md text-[#75778B]">Provider Services and Drugs</h3>
          ...
          <template v-else>
            <div class="p-3">
              <div class="relative bg-white p-4 rounded">
                <div
                  class="w-full bg-base-clr3 h-[3.5rem] focus-within:border-primary flex items-center rounded-lg overflow-hidden"
                >
                  <span
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="p-3">
                      ...
                      />
                      <span v-else
                        >ETB {{ item.userPrice.toLocaleString() }}</span
                      >
                    </td>
                  </tr>
                </tbody>
        ...
        <div class="bg-[#DFF1F1] h-auto p-4 rounded-md">
          ...
          <div class="flex justify-between items-center mt-3 text-sm">
            <div>
              <span
                >Showing
                {{ (selectedItemsPage - 1) * selectedItemsPerPage + 1 }} to
                {{
                  Math.min(
                    selectedItemsPage * selectedItemsPerPage,
